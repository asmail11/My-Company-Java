...
public class Go {
     ...
     public static String checkName(String username){
         try{
             ...
             Statement stmt;
             stmt = con.createStatement();
             String strname = "select * from users where "
                     + "username='" + username + "'";
             stmt.executeQuery(strname);
             while(stmt.getResultSet().next()){
                 con.close();
                 return username;
             }
     ...
     public static void fillToJTable(String tableNameOrSelectStatement, JTable table){
         try{
             ...
             Statement stmt = con.createStatement();
             ResultSet rs;
             String strSelectPart = tableNameOrSelectStatement.substring(0, 7).toLowerCase();
             String strSelect;
